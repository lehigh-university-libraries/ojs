...
<div class="page page_search">
	<div class="container-fluid">
		...
		{/foreach}
					...
					<div class="mb-4">
						<label class="form-label fw-bold" for="query">
							{translate key="search.searchFor"}
						</label>
						{block name=searchQuery}
							<div class="input-group input-group-lg">
								<span class="input-group-text"><i class="bi bi-search"></i></span>
								<input type="text" id="query" name="query" value="{$query|escape}" class="form-control" placeholder="{translate|escape key="common.search"}">
							</div>
						{/block}
					</div>

					<fieldset class="border rounded p-3 mb-4">
						<legend class="float-none w-auto px-2 h5 text-primary">
						...
						<div class="row g-3">
							...
							{if $searchableContexts}
								<div class="col-md-6">
									<label class="form-label" for="searchJournal">
										{translate key="search.journal"}
									</label>
									<select name="searchJournal" id="searchJournal" class="form-select">
										<option></option>
					...
					<div class="d-grid gap-2 d-md-flex justify-content-md-end">
						<button class="btn btn-primary btn-lg px-5" type="submit">
							<i class="bi bi-search me-2"></i>{translate key="common.search"}
						</button>
					</div>
				</form>
			</div>
